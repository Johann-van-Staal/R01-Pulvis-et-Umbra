...
define :breakdown? do (takt >= 64) && (takt < 80) end
define :groove? do (takt >= 8) && !breakdown? && (takt < 112) end
...
live_loop :offhat, sync: :puls do
  ...
  if groove?
    4.times do
      sleep 0.5
      sample :drum_cymbal_closed, amp: 0.3, rate: 0.75,
        finish: 0.6, lpf: 95
      sleep 0.5
    end
  else
...
with_fx :reverb, room: 0.7, mix: 0.35 do
  live_loop :akkorde, sync: :puls do
    ...
    if voll?
      use_synth :prophet
      akk = ((takt % 4) < 2) ? (chord :e3, :minor7) : (chord :f3, :major7)
      sleep 1.5
      play akk, release: 0.45, cutoff: 85, amp: 0.5
      sleep 2.0
      play akk, release: 0.35, cutoff: 80, amp: 0.4
      sleep 0.5
    else
      sleep 4
    end
